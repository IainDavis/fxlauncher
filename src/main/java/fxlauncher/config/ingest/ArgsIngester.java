...
/**
 * Concrete implemention of {@link ConfigurationIngester} that deduces {@link LauncherConfig}
 * settings from command-line arguments (or any other String array);
 *
 * @author idavis1
 */
public class ArgsIngester extends ConfigurationIngester {
  ...
  protected List<String> _ingest() {
    ...
    List<String> leftovers =
        Stream.of(argsSupplier.get()).filter(this::matchAndExtract).collect(toList());
    return leftovers;
  }
  ...
  private static final Function<String[], String> BEGIN_MSG =
      args ->
          String.format(
              "Ingesting command-line arguments: %s",
              Stream.of(args).collect(joining(",", "[", "]")));
  private static final BinaryOperator<String> MATCHED_OPT_MSG =
      (arg, opt) -> String.format("Matched argument '%s' with LauncherOption '%s'", arg, opt);
  private static final UnaryOperator<String> UNMATCHED_OPT_MSG =
      (arg) ->
          String.format(
              "No matching LauncherOption found for argument: '%s'. Sending to downstream application",
              arg);
}
